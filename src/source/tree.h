 ...
 public:
  ...
  class iterator {
   ...
   public:
    ...
    iterator(S21LLRBTree *tree, TreeNode *node) noexcept {
      node_ptr = node;
      tree_ptr = tree;
    }
  ...
  class const_iterator {
   ...
   public:
    ...
    // Constructor with paramaters
    const_iterator(const S21LLRBTree *tree, const TreeNode *node) noexcept
  ...
  /**
   * @brief Copy constructor. It copies S21LLRBTree object
   * @param other S21LLRBTree object to be copied
   */
  S21LLRBTree(const S21LLRBTree &other) {
    if (this != &other) {
      Clear();
   ...
   * It creates a new S21LLRBTree object and destroys an old
   * @param other S21LLRBTree object to be moved
   */
  S21LLRBTree(S21LLRBTree &&other) noexcept
      : root_{other.root_},
   ...
   * @param other S21LLRBTree object to be copied
   * @return Copied S21LLRBTree
   */
  S21LLRBTree &operator=(const S21LLRBTree &other) {
    if (this != &other) {
      Clear();
      const_iterator iter(&other, other.GetMinNodeSubtree(other.root_));
      for (; iter != other.cend(); ++iter) {
   ...
   * @param other S21LLRBTree object to be moved
   * @return Moved S21LLRBTree
   */
  S21LLRBTree &operator=(S21LLRBTree &&other) noexcept {
    if (this != &other) {
   ...
   */
  void Swap(S21LLRBTree &other) noexcept {
    std::swap(root_, other.root_);
    std::swap(null_node_, other.null_node_);
    std::swap(tree_size_, other.tree_size_);
  }
